...
class ImageUploadViewModel: ObservableObject {
    ...
    func uploadImage(completion: @escaping (Result<User?, Error>) -> Void) {
        // Convert UIImage to Data
        isLoading = true
        let imageDatas = uploadedImages.compactMap { image in
            return image.jpegData(compressionQuality: 1)
        ...
        // Create a multipart form data request
        guard let url = URL(string: "http://localhost:3000/api/v1/user/uploadPhoto/") else {
            errorMessage = "Invalid URL"
            isLoading = false
            return
        }
    ...
    func removeProfileImage(completion: @escaping (Result<User?, Error>) -> Void){
        isLoading = true
        
        guard let url = URL(string: "http://localhost:3000/api/v1/user/deleteProfilePhoto") else {
            errorMessage = "Invalid URL"
            isLoading = false
            return
        }
